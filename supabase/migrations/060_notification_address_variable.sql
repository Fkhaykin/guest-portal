-- Add {{address}} to all notification templates on existing hosts
...
SET notification_settings = notification_settings
  || jsonb_build_object(
      'cleaner_new_booking', jsonb_build_object(
        ...
        'message', 'New booking — {{property}}, {{address}}: {{guest}}, {{check_in}}–{{check_out}}, {{num_guests}} guest(s){{pets_text}}{{notes_text}}' || chr(10) || 'View: {{link}}'
      ),
...
ALTER TABLE host ALTER COLUMN notification_settings SET DEFAULT '{
  "cleaner_new_booking": {
    "enabled": true,
    "message": "New booking — {{property}}, {{address}}: {{guest}}, {{check_in}}–{{check_out}}, {{num_guests}} guest(s){{pets_text}}{{notes_text}}\nView: {{link}}"
  },
